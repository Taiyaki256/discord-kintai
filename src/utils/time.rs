...
pub fn parse_time_string(time_str: &str) -> Result<NaiveTime> {
    ...
    Err(anyhow::anyhow!(
        "Invalid time format. Use HH:MM (supports 00:00-47:59 for night shifts)"
    ))
}
...
pub fn combine_date_time_jst_with_day_offset(
    date: NaiveDate,
    time: NaiveTime,
    is_next_day: bool,
) -> DateTime<Utc> {
...
pub fn get_date_from_utc_timestamp(timestamp: DateTime<Utc>) -> NaiveDate {
    ...
    tracing::info!(
        "get_date_from_utc_timestamp: UTC={:?}, JST={:?}, Date={}",
        timestamp,
        jst_time,
        date
    );
